<?php
namespace COOL_PROCESS\feedback;

class cp_feedback{

	private $plugin_url = COOL_PROCESS_PLUGIN_URL;
	private $plugin_version = COOL_PROCESS_VERSION_CURRENT;
	private $plugin_name = 'Process Steps Template Designer';
	private $plugin_slug = 'cool_process';
    ...
    */
		...
		<div id="cool-plugins-deactivate-feedback-dialog-wrapper" class="hide-feedback-popup">
            ...
            <div id="cool-plugins-loader-wrapper">
				<div class="cool-plugins-loader-container">
                    <img class="cool-plugins-preloader" src="<?php echo COOL_PROCESS_PLUGIN_URL?>feedback/images/cool-plugins-preloader.gif">
                </div>
            </div>
    ...
    function submit_deactivation_response(){
        if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( $_POST['_wpnonce'], '_cool-plugins_deactivate_feedback_nonce' ) ) {
			...
			$reason = $_POST['reason'];
			$message = $_POST['message']==''?'N/A':$_POST['message'];
			
			$response = wp_remote_post( $this->feedback_url , [
                'timeout' => 30,
                'body' => [
                    'plugin_version' => (string) $this->plugin_version,
