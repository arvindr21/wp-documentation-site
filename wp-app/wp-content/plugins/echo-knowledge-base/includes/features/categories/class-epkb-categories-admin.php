...
class EPKB_Categories_Admin {
	...
	public function update_categories_sequence( $updated_term_id='', $taxonomy='' ) {
		...
		if ( $categories_order_method == 'user-sequenced' ) {
			...
			$new_cat_ids_obj = $config_seq->update_categories_order( $kb_id, $orig_sequence, $new_cat_ids_obj );
			if ( $new_cat_ids_obj === false ) {
				return false;
			}
		}

		EPKB_Utilities::save_kb_option( $kb_id, self::KB_CATEGORIES_SEQ_META, $new_cat_ids_obj->ids_array, true );
